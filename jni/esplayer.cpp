...
#define LOG_TAG "khadas_hw_decode"
...
int osd_blank(char *path, int cmd)
{
    ...
    ALOGD("osd_blank enter fd %d",fd);
    if (fd >= 0) {
        sprintf(bcmd, "%d", cmd);
        write(fd, bcmd, strlen(bcmd));
	    ALOGD("osd_blank %s",path);
        close(fd);
        return 0;
    }
...
int send_buffer_to_device(char *buffer, int Readlen)
{
    ...
    do {
        ...
        if (ret < 0) {
            if (errno != EAGAIN) {
                ALOGD("write data failed, errno %d\n", errno);
                return -1;
            } else {
                continue;
            }
            ...
            if (meta_type == OBU_METADATA_TYPE_ITUT_T35) {
                ...
                if ((p[0] == 0xb5) /* country code */
                    ...
                    if (p[11] & 0x10) {
                        ...
                        if (p[12] & 0x08) {
                            ALOGD("\tmeta rpu in obu exceed 512 bytes\n");
                            break;
                        }
                ...
                ALOGD("\t\thdr10 primaries[r,g,b] = \n");
                for (i = 0; i < 3; i++) {
                    ALOGD("\t\t %x, %x\n",
                        (p[i * 4] << 8) | p[i * 4 + 1],
                        (p[i * 4 + 2] << 8) | p[i * 4 + 3]);
                }
                ALOGD("\t\twhite point = %x, %x\n", (p[12] << 8) | p[13], (p[14] << 8) | p[15]);
...
int ivf_write_dat(FILE *src_fp, uint8_t *src_buffer)
{
    ...
    meta_buffer = (uint8_t *)calloc(1, 1024);
    if (!meta_buffer) {
        ALOGD("fail to alloc meta buf\n");
        return -1;
    }
    if (fread(src_buffer, 1, 32, src_fp) != 32) {
        ALOGD("read input file error!\n");
        return -1;
    }
    p_size = (unsigned int *)(src_buffer + 24);
    process_count = *p_size;
    ALOGD("frame number = %d\n", process_count);
    while (frame_count < process_count) {
        ...
        ALOGD("frame %d, size %d\n", frame_count, src_frame_size);

        if (fread(src_buffer, 1, src_frame_size, src_fp) != src_frame_size) {
            ALOGD("read input file error %d!\n", src_frame_size);
            break;
        }

        dst_buffer = (uint8_t *)calloc(1, src_frame_size + 4096);
        if (!dst_buffer) {
            ALOGD("failed to alloc frame buf\n");
            break;
        }
        dst_frame_size = 0;
        meta_size = 0;
...
check_av1:
        if (nal_unit_type == OBU_TILE_GROUP) {
            ...
            if (nal_unit_type == OBU_FRAME) { /*have tile group in this OBU*/
                ...
                obu_frame_frame_head_come_after_tile == 0) {
            decoding_data_flag = 0;
            ALOGD("Warning, OBU_REDUNDANT_FRAME_HEADER come without OBU_FRAME or OBU_FRAME_HEAD\n");
        }
    }
...
int GetAnnexbNALU (FILE* fe, NALU_t *nalu, int format)
{
    ...
    if ((Buf = (unsigned char*)calloc (nalu->max_size , sizeof(char))) == NULL)
        ALOGD ("GetAnnexbNALU: Could not allocate Buf memory\n");
    ...
    {
        free(Buf);
        ALOGD("GetAnnexbNALU: Cannot fseek in the bit stream file");
    }
...
fill_data:
    ...
    if (nalu->len > MAX_SIZE) {
        ALOGD("%d: Error: to many data to copy %d\n", __LINE__, nalu->len);
        exit(0);
    }
...
int hardware_decode(JNIEnv *env, jclass clazz __unused,jstring file_name, jint width,jint height,jint fps,jint mode,jint format)
{
    ALOGD("hardware_decode enter file_name:%s width:%d height:%d fps:%d mode %d format %d\n",file_name,width,height,fps,mode,format);
    char *buffer = NULL;
    ...
    if (NULL == buffer) {
        ALOGD("malloc write buffer fail\n");
        return -1;
    }
    ...
    if (vpcodec->video_type == VFORMAT_H264) {
        ...
        vpcodec->am_sysinfo.param = (void *)(EXTERNAL_PTS | SYNC_OUTSIDE);
            ...
            ALOGD("No subformat for vc1, take the default VIDEO_DEC_FORMAT_WVC1\n");
            vpcodec->am_sysinfo.format = VIDEO_DEC_FORMAT_WVC1;
    } else if (vpcodec->video_type == VFORMAT_MPEG4) {
            ALOGD("No subformat for mpeg4, take the default VIDEO_DEC_FORMAT_MPEG4_5\n");
    ...
    filename = (char *)env->GetStringUTFChars(file_name, NULL);;
    ALOGD("file %s to be played\n", filename);
    ...
    if (ret != CODEC_ERROR_NONE) {
        fclose(fp);
        ALOGD("codec init failed, ret=-0x%x", -ret);
        goto osd_restore;
    }
    ALOGD("video codec ok!\n");
    ...
    if (is_video_file_type_ivf(fp, vpcodec->video_type, buffer)) {
        ALOGD("input video file is ivf with av1.\n");
    ...
    do {
        ...
        if (ret != 0) {
            ALOGD("vcodec_get_vbuf_state error: %x\n", -ret);
            goto error;
        }
...
static JNINativeMethod sMethods[] = {
    {"native_hardware_decode", "(Ljava/lang/String;IIIII)I", (void*)hardware_decode},
};
...
JNI_OnLoad(JavaVM* vm, void* reserved __unused)
{
    ...
    if (registerNativeMethods(env,"com/khadas/khadashwdecoderdemo/KhadasHwDecodeManager", sMethods, NELEM(sMethods)) < 0)
    return -1;


    clazz = (*env).FindClass("com/khadas/khadashwdecoderdemo/KhadasHwDecodeManager");
    if (clazz == NULL) {
        log_error("FindClass com/khadas/khadashwdecoderdemo/KhadasHwDecodeManager failed");
        return -1;
    }
